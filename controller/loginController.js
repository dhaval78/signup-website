...
var url='mongodb://dhaval78:getrektson123@ds227168.mlab.com:27168/ds'
...
var urlencodedParser = bodyParser.urlencoded({extended:false});
	...
	// Login TO DB==================================================================
		app.post('/demo',urlencodedParser,function(req,res){
		 ...
		 adb.collection('user').findOne({ name: req.body.name}, function(err, user) {
							 if(user ===null){
	...
	//register profile to MongoDB================================================================
		app.post('/completeprofile',urlencodedParser,function(req,res){
		 ...
		 var jsonObj = JSON.parse(obj);
				MongoClient.connect(url, function(err, db) {
					const adb=db.db('ds');
				adb.collection("user").insertOne(jsonObj, function(err, res) {
			 if (err) throw err;
